...
colors  = [kBlack,kRed+2,kBlue+2,kGreen+3,kYellow+2,kRed+2,kAzure-2,kGreen-8,kOrange+3,kYellow+2,kRed+2,kBlue+2,kGreen-8,kOrange+3,kYellow+2,kRed+2,kBlue+2,kGreen-8,kOrange+3,kYellow+2,kRed+2,kBlue+2,kGreen-8,kOrange+3,kYellow+2,kRed+2,kBlue+2,kGreen-8,kOrange+3]
...
def Make1DPlot(name,outdir,histos,legends,oFext,xtitle="",ytitle="",ymin=0,ymax=1,noErrors=False,RebinFactor=0,runNumber="",additionalText=[],RatioType="Alternate",LogX=False,LogY=False,RatioMin=0.25,RatioMax=1.75,WriteMean=False,Normalise=False,doFit=False,drawOptions="hist",Xmin=-999,Xmax=-999,legCorner=0):
    ...
    for ih in xrange(len(histos)):
        ...
        if ih==0:
            ...
            if noErrors:
                histos[ih].Draw("psame "+drawOptions)
            else:
                histos[ih].Draw("pesame "+drawOptions)
...
def Make2DPlots(name,outdir,histolist,legends=[],colors2d=colors,
                    xtitle="",ytitle="",ztitle="",
                    text="",zmin=-999,zmax=-999,
                    style=""):
    oFext=".pdf"
    if not os.path.exists(outdir):
        os.mkdir(outdir)
    ...
    for ih in range(0,len(histolist)):
        ...
        histolist[ih].SetMarkerStyle(20)
        histolist[ih].SetMarkerSize(0.4)
        histolist[ih].SetMarkerColor(colors2d[ih])
        histolist[ih].SetLineColor(colors2d[ih])
        ...
        histolist[ih].GetXaxis().SetTitleOffset(
            histolist[ih].GetXaxis().GetTitleOffset()*0.3)
        ...
        if style == "colz":
            ...
            if (ih==0):
                print "draw first", histolist[ih], ih
                histolist[ih].Draw()
            else:
                print "draw", histolist[ih], ih
    ...
    if len(legends)>0:
        ...
        for i_leg in xrange(len(legends)):
            #print "Adding Entry",i_leg, legends[i_leg] 
            leg.AddEntry(histolist[i_leg],legends[i_leg],"lpf") 
            pass
        leg.Draw()
        pass
